...
Keep refresh token validity at 30 days so sessions are convenient without
staying valid for years:
...
By default this reads private card data from `private/cards.private.ts` and
writes it to the DynamoDB `Card` table. You can override the source path:

```bash
CARD_SOURCE_PATH=./private/cards.private.ts node scripts/seed-cards.mjs
```

Keep `private/cards.private.ts` out of version control if the repo is public.
...
- [ ] Private card source is not tracked in the public repo
